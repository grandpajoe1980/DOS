...
|---|---|---|---|---|---|
| M0 — Foundations and contracts | Reproducible iOS project/CI plus an accepted v1 interface boundary and deterministic fixtures. | PR-008, PR-010; all FRs at contract level | Documentation on `main` | #5 is green; draft PR #12/#6 is reviewed, compatible, validated in CI, and accepted by database/security, iOS, web, and quality owners. | In progress |
| M1 — Authoritative data and tenant security | Local Supabase/PostGIS reset produces a two-tenant source of truth with constraints, RLS, seed data, audit, and policy mutations. | FR-002; backend prerequisites for all FRs | Accepted relevant #6 contracts; CI | #7 migration/reset/recovery, allow/deny matrix, hard-cap race, idempotency, public-location, and secret-leak tests pass. | Blocked |
| M2 — Deployable surfaces and identity | A clean clone launches iOS and web shells against non-production services; adults can authenticate, manage a profile, create an organization, and use scoped roles. | FR-001–FR-004; NFR-001–NFR-005 | #5, accepted #6, tenant controls from #7; provider setup for live integration | #8–#10 critical identity journeys, secure sessions, accessibility, stale-session/cross-tenant denial, and environment separation pass. | Blocked |
| M3 — Event authoring and discovery | Organizers create, preview, and publish reusable programs and dated sites/shifts/tasks; public iOS/web discovery exposes only approved fields. | FR-101–FR-102; PR-002, PR-007 | M1–M2 | Authoring state transitions, time-zone/DST, public filtering, discovery performance/accessibility, audit, and rollback pass. | Blocked |
| M4 — Registration and consent | Adults register individuals/teams/dependents; active participant-specific evidence and capacity policy are server enforced. | FR-103, FR-106; prerequisites for FR-104–FR-105; PR-003–PR-004 | M3; approved legal-document/guardian policy before public pilot | Guardian, consent-version, eligibility, duplicate/retry, hard-cap concurrency, accessibility, and privacy tests pass. | Blocked |
| M5 — Assignment and event-day operations | Organizers plan/confirm assignments; participants and leads use directions, rosters, attendance, announcements, incidents, and optional Safety Sharing under intermittent connectivity. | FR-104–FR-105, FR-201–FR-206 | M4; notification integration for announcement acceptance | Assignment constraints, event-day load, encrypted offline reconciliation, permission boundaries, redaction, expiry, and operational runbooks pass. | Blocked |
...
|---|---|---|---|---|---|
| P0 | #6 / PR #12 — Versioned v1 contracts | Lead Architect / contract owner | Review 26-path OpenAPI, schemas, negative fixtures, unknown enums, public/private separation, idempotency, compatibility, and PG-001's separate member-feed/report-hide/public-gallery semantics; record required owner approvals. | None | Under review |
| P0 | #5 — Xcode project and CI quality gates | Integration/Platform Developer | Checked-in iOS project, SwiftPM and simulator tests, contract validation, lint/static/dependency/secret checks, PR template, ownership map, reproducible README. | None; coordinate contract CI command with PR #12 | Ready |
| P0 | #11-A — Independent contract/CI test plan | Automated Test Engineer | Add compatibility/negative-fixture plan and known-failure proofs; define deterministic artifacts, ownership, duration, and flaky-test policy. | Can design now; execution joins #5/#6 | Ready |
